(ns rest-of-advent-24.utils.macros)

(defn ^:private is-explicit-return? [r]
  (and (seq? r)
       (= (first r) 'return)))

(defn ^:private unwrap-explicit-return [r]
...
(defmacro boop ;; boomer loop
  {:clj-kondo/ignore true}
  [init cond finally & body]
  `(let [x# (atom nil)]
    (loop ~init
     (when ~cond
       (reset! x# (do ~@body))
